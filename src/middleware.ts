...
const requireLogin = defineMiddleware(async (context, next) => {
  ...
  // Admin routes (src/pages/api/admin/*) authenticate themselves with a
  // secret header, not a JB session — the restore endpoint in particular has
  // to run before any real data exists, so there is no session to hold yet.
  // Skipping the JB check here only decides this prefix doesn't need one;
  // every route under it still does its own secret comparison (and, for
  // restore, its own "table already has rows" guard), so nothing here
  // weakens or duplicates those checks.
  if (pathname.startsWith('/api/admin/') && context.request.headers.has('x-admin-secret')) {
    return next();
  }
